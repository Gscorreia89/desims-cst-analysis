...
```
# Version with partR2, new library
```{r}
fit_lmm_r2estimates <- function(dataset, var) {
    tryCatch({
...
Iterate through the DESI Positive ionisation mode dataset and obtain the model R squares and the partial R squares per covariate
```{r}
...
# Generate a stacked barplot barplot with the 
```{r}
...
immune_markers <- read.xlsx('../../Data/Extended Data Table 3 ImmuneMarkers.xlsx', sheetIndex=1)#
...
plotR2(ldepl_vmet_neg, 113)
```



```{r}
# Subset the variables containing r2 measures for the fixed effects
r2_values_set <- r2_estimates_desi_neg_vmet[, c("Metabolite", "CST", "GestationalAge", "Age", "BMI", "Ethnicity", "R2Conditional_nsj")]

r2_values_set[, 8] <- r2_values_set[, 7] - rowSums(r2_values_set[, 2:6])
r2_values_set[r2_values_set[, 8] < 0, 8] <- 0

colnames(r2_values_set)[8] <- 'R2'

melted <- melt(r2_values_set, 'Metabolite')

melted$Metabolite <- round(as.numeric(as.character(melted$Metabolite)), digits=3)

#melted_sort <- melted[order(- as.numeric(melted$value), melted$Metabolite, melted$variable), ]
melted$value <- as.numeric(melted$value)
...
ggplot(plotDFrame, aes(fill=variable, y=as.numeric(value), x=factor(Metabolite, r2_covar$Metabolite[1:top_n]))) +  geom_bar(stat='identity', position='stack', alpha=1) + ylab('R2') + xlab('Metabolite') + theme( axis.text.x = element_blank(),
  axis.ticks.x = element_blank())
#+ geom_text(aes(label = as.numeric(value)), size = 2) 
```

```{r}
ggplot(r2_estimates_desi_neg_vmet) + geom_histogram(aes(x=Adjusted_ICC, fill=I('steelblue2'), col=I('red')))

ggplot(r2_estimates_desi_neg_vmet) + geom_col(aes(x=Metabolite, y=Adjusted_ICC, fill=I('steelblue2'))) + theme( axis.text.x = element_blank(), axis.ticks.x = element_blank())
```

```{r}

ms_profile_vmet2 <- desi_neg_vmet2[, desineg_first_metabolite_idx_vmet2:dim(desi_neg_vmet2)[2]]
metadata <- desi_neg_vmet2[, c('CST', 'Ethnicity', 'GestationalAge', 'BMI', 'Age', 'SubjectID')]
library(vegan)
permanova <- adonis(ms_profile_vmet2 ~ CST + GestationalAge + Ethnicity + BMI + Age + SubjectID,
               data = metadata, permutations=250, method = "euclidean")
