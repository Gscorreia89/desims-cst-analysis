...
Define a set of helper functions to handle the main comparisons.
These are:
  - CST I vs III:
  - CST I vs IV:
  - CST III vs IV:
  - Multinomial Comparison of CST I, II, III, IV, V:
  - Multinomial Comparison of CST I, III and IV
...
# All vs All,including II and V
# "Multinomial" classification model, all classes at the same time
random_forest_comparison_AllvsAll <- function(dataset, first_metabo_idx) {
  dataset_train <- dataset[, c('CST', colnames(dataset)[first_metabo_idx:dim(dataset)[2]])]
  # remove rare levels as
  dataset_train$CST <- droplevels(dataset_train$CST)
  dataset_train[is.na(dataset_train)] <- 0
  
  fit_control <- trainControl(method = "repeatedcv", repeats=15, classProbs = TRUE,
                           number = 5, summaryFunction=multiClassSummary, 
                           preProcOptions = c('center', 'scale', 'nzv'), savePredictions=TRUE)
  rf_fit <- train(CST ~ ., 
                data = dataset_train, method='rf', trControl=fit_control, metric='AUC', tuneGrid=data.frame(mtry=50))
...
Can DESI-MS data be used to identify which swab samples come from women with a Lactobacillus Depleted State?
To find out, we fit RandomForest Classifiers to discriminate between Lactobacillus Dominant or Depleted States. To validate the model, a repeated (n=15) stratified 7-fold cross-validation scheme is applied. Area under the curve and other metrics are calculated using samples from the test-sets.

In this cell, the "Lactobacillus Dominant" or "Depleted" criteria used is based on the CST Hierarchical clustering assignment. Samples with a CST IV or VI (B.breve) are considered "Depleted".
